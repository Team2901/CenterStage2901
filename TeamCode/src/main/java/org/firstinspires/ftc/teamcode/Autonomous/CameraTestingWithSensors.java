...
public class CameraTestingWithSensors extends LinearOpMode implements OpenCvCamera.AsyncCameraOpenListener {
    ...
    CameraTestingWithSensors.AutoState autoState;
...
//        cameraTimer.startTime();
    ...
    void _loop() {
        ...
        if (autoState == CameraTestingWithSensors.AutoState.CAMERA_WAIT) {
            if (cameraTimer.time(TimeUnit.SECONDS) < 5) {
                ...
                autoState = CameraTestingWithSensors.AutoState.CAMERA_DETECTION;
            }
        } else if (autoState == CameraTestingWithSensors.AutoState.CAMERA_DETECTION) {
            if (pipeline.spikeMark == 1) {
                autoState = CameraTestingWithSensors.AutoState.MOVE_1;
            } else if (pipeline.spikeMark == 2) {
                autoState = CameraTestingWithSensors.AutoState.MOVE_2;
            } else if (pipeline.spikeMark == 3) {
                autoState = CameraTestingWithSensors.AutoState.MOVE_3;
            }
        } else if (autoState == CameraTestingWithSensors.AutoState.MOVE_1) {
        ...
        } else if (autoState == CameraTestingWithSensors.AutoState.MOVE_2) {
            if (!robot.frontLeft.isBusy() && !robot.frontRight.isBusy() && !robot.backLeft.isBusy() && !robot.backRight.isBusy()) {
                moveInches(43);
        ...
        } else if (autoState == CameraTestingWithSensors.AutoState.MOVE_3) {
            if (!robot.frontLeft.isBusy() && !robot.frontRight.isBusy() && !robot.backLeft.isBusy() && !robot.backRight.isBusy()) {
                moveInches(31);
                strafe(-6,0,0,0,0);
...
//                turnByTicks(2400); //more than 180
        ...
        }else if (autoState == CameraTestingWithSensors.AutoState.STOP) {
            if (!robot.frontLeft.isBusy() && !robot.frontRight.isBusy() && !robot.backLeft.isBusy() && !robot.backRight.isBusy()) {
